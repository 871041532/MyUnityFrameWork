// 物体坐标系切变测试shader
Shader "MathLearn/X轴切变测试"
{
    Properties
    {
        m_DiffuseColor("Diffuse Color", Color) = (1,1,1,1)
        m_ShearS("切变s值", Float) = 0
        m_ShearT("切边t值", Float) = 0
    }
    SubShader
    {
        Pass{
            ...
            {
                 v2f o;
                 float3x3 rotationMatrix = float3x3(float3(1, m_ShearS, m_ShearT), float3(0, 1, 0), float3(0, 0, 1));
