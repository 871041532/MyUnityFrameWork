...
Shader "MathLearn/缩放测试"
{
    Properties
    {
        m_DiffuseColor("Diffuse Color", Color) = (1,1,1,1)
        m_ScaleX("X轴缩放", Float) = 0
        m_ScaleY("Y轴缩放", Float) = 0
        m_ScaleZ("Z轴缩放", Float) = 0
    }
    SubShader
    {
        Pass{
            ...
            {
                 v2f o;
                 float3x3 rotationMatrix = float3x3(float3(m_ScaleX, 0, 0), float3(0, m_ScaleY, 0), float3(0, 0, m_ScaleZ));
