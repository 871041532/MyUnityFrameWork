...
Shader "MathLearn/X轴旋转测试"
{
    ...
    SubShader
    {
        Pass{
            ...
            {
                 v2f o;
                 float cosValue = cos(m_Rad);
                 float sinValue = sin(m_Rad);
                 float3x3 rotationMatrix = float3x3(float3(1, 0, 0), float3(0, cosValue, sinValue), float3(0, -sinValue, cosValue));
