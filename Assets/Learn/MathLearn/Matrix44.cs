...
namespace MathLearn
{
    public struct Matrix44
    {
        ...
        // 默认乘法是点
        public static Vector4 operator *(Vector4 p, Matrix44 m)
        {
            float x = p.x * m[0, 0] + p.y * m[1, 0] + p.z * m[2, 0] + p.w * m[3, 0];
            float y = p.x * m[0, 1] + p.y * m[1, 1] + p.z * m[2, 1] + p.w * m[3, 1];
            float z = p.x * m[0, 2] + p.y * m[1, 2] + p.z * m[2, 2] + p.w * m[3, 2];
            float w = p.x * m[0, 3] + p.y * m[1, 3] + p.z * m[2, 3] + p.w * m[3, 3];
