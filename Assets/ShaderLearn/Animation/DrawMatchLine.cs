...
class DrawMatchLine:MonoBehaviour
{
    private GameObject m_root = null; 
    public GameObject m_origin = null;
    ...
    private void OnGUI()
    {
        ...
        {
            m_lineHandles.Push(DrawLine(new Vector3(Random.Range(-20f, 20f), Random.Range(-20f, 20f), Random.Range(-20f, 20f)), new Vector3(Random.Range(-20f, 20f), Random.Range(-20f, 20f), Random.Range(-20f, 20f)), new Color(Random.Range(0f,1f), Random.Range(0f, 1f), Random.Range(0f, 1f),1), Random.Range(0f, 8f)));
        }
        if (GUI.Button(new Rect(100, 0, 100, 50), "回收"))
        {
            if (m_lineHandles.Count>0)
            {
                BackLine(m_lineHandles.Pop());
            }
        }
    }
